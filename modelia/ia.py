...
detection_interval = 0.5  # Réduit de 2 à 0.5 secondes pour des détections plus fréquentes
...
def init_camera():
    ...
    # Liste des méthodes d'initialisation à essayer
    camera_methods = [
        (0, cv2.CAP_ANY),  # Index 0 avec backend par défaut
        (0, cv2.CAP_V4L2),  # Index 0 avec V4L2
        (1, cv2.CAP_ANY),  # Index 1 avec backend par défaut
        (1, cv2.CAP_V4L2),  # Index 1 avec V4L2
        (2, cv2.CAP_ANY),  # Index 2 avec backend par défaut
        (2, cv2.CAP_V4L2),  # Index 2 avec V4L2
    ]

    for index, backend in camera_methods:
        try:
            print(f"Tentative avec index {index} et backend {backend}")
            ...
            if camera.isOpened():
                ...
                if ret and test_frame is not None:
                    print(f"✅ Caméra initialisée avec succès (index {index}, backend {backend})")
                    print(f"Résolution: {test_frame.shape}")
                    return True
                else:
                    print(f"❌ La caméra ne peut pas lire d'images (index {index})")
                    camera.release()
                    camera = None
            else:
                print(f"❌ Impossible d'ouvrir la caméra (index {index})")

        except Exception as e:
            print(f"❌ Erreur lors de l'initialisation de la caméra (index {index}): {str(e)}")
            if camera is not None:
                try:
                    camera.release()
                except:
...
def process_frame():
    ...
    def run_detection(frame_copy):
        ...
        try:
            if is_detecting and model_plante and model_maladie and (time.time() - last_detection_time) >= detection_interval:
                # Détection de la plante
                results_plante = model_plante(frame_copy, verbose=False)
                if len(results_plante) > 0 and len(results_plante[0].boxes) > 0:
                    # Le reste de la logique de détection reste identique
                    ...
                    if conf_plante > SEUIL_CONF_PLANTE:
                        if classe_plante.lower() in [f.lower() for f in feuilles_saines]:
                            for box in results_plante[0].boxes:
                                ...
                                cv2.putText(frame_copy, "PLANTE SAINE", (x1, y1-10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)

                            image_path = sauvegarder_image(frame_copy, "PLANTE SAINE")
                            if image_path:
                                current_detection = {
                                    'status': 'detection',
                                    'name': 'PLANTE SAINE',
                            ...
                            results_maladie = model_maladie(frame_copy, verbose=False)
                            if len(results_maladie) > 0 and len(results_maladie[0].boxes) > 0:
                                ...
                                if conf_maladie > SEUIL_DETECTION_MALADIE:
                                    for box in results_plante[0].boxes:
                                        x1, y1, x2, y2 = map(int, box.xyxy[0])
                                        cv2.rectangle(frame_copy, (x1, y1), (x2, y2), (0, 255, 0), 2)
                                        cv2.putText(frame_copy, "Plante", (x1, y1-10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)

                                    for box in results_maladie[0].boxes:
                                        x1, y1, x2, y2 = map(int, box.xyxy[0])
                                        cv2.rectangle(frame_copy, (x1, y1), (x2, y2), (0, 0, 255), 2)
                                        cv2.putText(frame_copy, f"{classe_maladie} ({conf_maladie:.2f})", (x1, y1-10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)

                                    image_path = sauvegarder_image(frame_copy, classe_maladie)
                                    if image_path:
                                        current_detection = {
                                            'status': 'detection',
                                            'name': classe_maladie,
    ...
    while is_running:
        ...
        if not ret or frame is None:
            ...
            time.sleep(0.01)  # Réduit le délai d'attente
            continue
        ...
        # Lancer la détection dans un thread séparé si nécessaire
        if detection_thread is None or not detection_thread.is_alive():
            detection_thread = threading.Thread(target=run_detection, args=(frame_display,))
        ...
        time.sleep(0.01)  # Réduit le délai d'attente pour plus de fluidité
...
def start_detection():
    global is_detecting, is_running, frame_thread
    try:
        if not is_running:
            if not init_camera():
